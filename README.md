...
##### Pipeline de Données (ETL)
    ...
    Pré-traitement séparé (preprocess.py) : Nous avons isolé l'étape de nettoyage des données brutes (.ttl).
...
Augmentation des Partitions : Passage de 20 à 200 partitions.
    ...
    Partitionnement par Source (src) :
      Essentiel pour respecter la consigne "éviter le shuffle dans la boucle". En regroupant les liens par URL source dès le début, la structure du graphe reste fixe sur les nœuds. Seuls les scores (ranks) circulent sur le réseau.
